...
public class Produto {
  ...
  public void entryItem(Integer amount) {
    if (amount == null || amount <= 0) {
        throw new RuntimeException("Amount must be greater than 0.");
    }
    
    // Proteção contra nulo no campo da classe
    int atual = (this.quantidadeItens == null) ? 0 : this.quantidadeItens;
    this.quantidadeItens = atual + amount;
}

public void exitItem(Integer amount) {
    if (amount == null || amount <= 0) {
    ...
    if (atual < amount) {
        throw new RuntimeException("Stock insufficient! Available: " + atual);
    }
